...
class DenoiserMemory(torch.nn.Module):
    ...
    def __init__(self, num_nodes: int, raw_msg_dim: int, memory_dim: int,
        ...
        super(DenoiserMemory, self).__init__()
    ...
    def forward(self, n_id: Tensor, noise: Tensor) -> Tuple[Tensor, Tensor]:
        """Returns, for all nodes :obj:`n_id`, their current memory and their
        last updated timestamp."""
        if self.training:
            memory, last_update = self.__get_updated_memory__(n_id, noise)
        else:
            memory, last_update = self.memory[n_id], self.last_update[n_id]

        return memory, last_update

    def update_state(self, src, dst, t, raw_msg, noise):
        """Updates the memory with newly encountered interactions
        :obj:`(src, dst, t, raw_msg)`."""
        n_id = torch.cat([src, dst]).unique()

        if self.training:
            self.__update_memory__(n_id, noise)
    ...
    def __update_memory__(self, n_id, noise):
        memory, last_update = self.__get_updated_memory__(n_id, noise)
        self.memory[n_id] = memory
        self.last_update[n_id] = last_update

    def __get_updated_memory__(self, n_id, noise):
        ...
        aggr = self.aggr_module(
            msg, self.__assoc__[idx], t, n_id.size(0))
    ...
    def __compute_msg__(self, n_id, msg_store, msg_module, noise):
    ...
    def train(self, mode: bool = True):
        ...
        if self.training and not mode:
            ...
            self.__reset_message_store__()
        super(DenoiserMemory, self).train(mode)
